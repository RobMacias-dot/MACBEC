...
2. mppt_count: Rango válido 1-12, regex evita porcentajes (99%)
3. max_pv_power_w: Nunca None - inferencia con fallback físico mínimo
4. Validaciones en tiempo de extracción para mayor robustez
...
def aggressive_voltage_extraction(text: str) -> Optional[float]:
    ...
    Busca patrones comunes de voltaje en tablas de paneles.
    """
    if not text:
        return None
...
# -----------------------------------------------------------------------------
# Assembler + validation/inference - V8.1: CORRECCIÓN CRÍTICA
# -----------------------------------------------------------------------------
...
def validate_and_infer_panel(out: Dict[str, Any], full_text: Optional[str] = None) -> Dict[str, Any]:
    """
    CORRECCIÓN CRÍTICA: Ahora recibe full_text como parámetro explícito
    """
    ...
    # GARANTÍA FINAL: voc_v NUNCA será None
    if out.get("voc_v") is None:
        out["voc_v"] = 45.0
        logger.warning("GARANTÍA: voc_v forzado a 45.0V (último recurso)")
...
def process_pdf(pdf_path: str) -> Dict[str, Any]:
    ...
    if doc_type == "panel":
        ...
        if out.get("voc_v") is None:
            out["voc_v"] = 45.0  # Valor por defecto seguro > 40
