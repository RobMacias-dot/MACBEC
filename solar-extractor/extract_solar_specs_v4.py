...
# -*- coding: utf-8 -*-
"""
extract_solar_specs_v4.py (versión híbrida + debug detallado)
-------------------------------------------------------------
Extractor de especificaciones para paneles e inversores desde PDFs.

- Intenta primero TEXTO NATIVO (PyMuPDF, luego pdfplumber).
...
- Modo debug detallado (--debug):
    * Muestra por campo qué regex se probó y cuál coincidió.
- Si un campo no se encuentra:
    * En el CSV se escribe "no_se_encuentra".
    * En consola (con --debug) se listan los campos faltantes por PDF.
...
PANEL_FIELDS = [
    "fabricante", "modelo",
    "potencia_nominal_W", "Voc_V", "Isc_A", "Vmp_V", "Imp_A",
...
def debug_regex_field(label: str, patterns, text: str, debug_mode: bool) -> Optional[str]:
    """
    Muestra en consola cuál patrón coincide y cuál no para un campo.
    Devuelve el valor encontrado (string) o None.
    """
    if not debug_mode:
        return find_first(patterns, text)

    print(f"\n[DEBUG][REGEX] Campo: {label}")
    ...
    for i, p in enumerate(patterns, start=1):
        ...
        if m:
            if m.groups():
                val = next((g for g in m.groups()
                            if g and re.search(r"[0-9]", g)), m.group(0))
            else:
                val = m.group(0)
            snippet = p.pattern.replace("\n", " ")
            if len(snippet) > 80:
                snippet = snippet[:77] + "..."
            print(f"   ✔️ Patrón {i}: {snippet} → MATCH = {val}")
            return val
        else:
            snippet = p.pattern.replace("\n", " ")
            if len(snippet) > 80:
                snippet = snippet[:77] + "..."
            print(f"   ❌ Patrón {i}: {snippet} → no coincide")

    print("Resultado final: no se encontró coincidencia.")
    return None


def mark_missing_and_warn(device_type: str,
                          filename: str,
                          row: Dict,
                          fieldnames: List[str],
                          debug_mode: bool) -> Dict:
    """
    Rellena campos None/"" con "no_se_encuentra" (excepto fabricante/modelo/url)
    y, si hay campos faltantes, los reporta en consola en modo debug.
    """
    missing = []
    fixed = {}
    ...
    for k in fieldnames:
        v = row.get(k)
        if v is None or v == "":
            # estos sí pueden faltar sin que cuente como "warning fuerte"
            if k not in ("fabricante", "modelo", "ficha_tecnica_url"):
                missing.append(k)
            fixed[k] = "no_se_encuentra"
        else:
            fixed[k] = v

    if missing and debug_mode:
        print(f"[WARN][{device_type}][{filename}] Campos sin dato: {', '.join(missing)}")
...
PAT_DIMENSIONS = [
    # Dimension / Dimensions 1956 x 992 x 50 mm con tolerancias opcionales
    re.compile(
...
def extract_panel(text: str, debug_mode: bool) -> Dict[str, Optional[str]]:
    out = {k: None for k in PANEL_FIELDS}

    # Potencia nominal
    raw_pmax = debug_regex_field("Pmax / Potencia Panel", PAT_POT_PANEL, text, debug_mode)
    pmax_val = to_float_safe(raw_pmax)
    if pmax_val is not None and raw_pmax:
        whole = raw_pmax.lower()
        # Nota: aquí no tenemos el match completo, pero el valor ya está bien;
        # la conversión a kW se maneja arriba en debug_regex_field si se necesitara.
    out["potencia_nominal_W"] = pmax_val

    # Parámetros eléctricos básicos
    out["Voc_V"] = to_float_safe(debug_regex_field("Voc", PAT_VOC, text, debug_mode))
    out["Isc_A"] = to_float_safe(debug_regex_field("Isc", PAT_ISC, text, debug_mode))
    out["Vmp_V"] = to_float_safe(debug_regex_field("Vmp", PAT_VMP, text, debug_mode))
    out["Imp_A"] = to_float_safe(debug_regex_field("Imp", PAT_IMP, text, debug_mode))
    ...
    # Peso
    w = debug_regex_field("Peso", PAT_WEIGHT, text, debug_mode)
    out["peso_kg"] = to_float_safe(w) if w else None

    # Tensión de sistema y coeficientes
    out["tension_sistema_max_V"] = to_float_safe(
        debug_regex_field("Tensión sistema máx", PAT_SYSTEM_VOLT, text, debug_mode)
    )
    out["coef_temp_Voc_pct_C"] = to_float_safe(
        debug_regex_field("Coef. temp Voc", PAT_TEMP_COEF_VOC, text, debug_mode)
    )
    out["coef_temp_Pmax_pct_C"] = to_float_safe(
        debug_regex_field("Coef. temp Pmax", PAT_TEMP_COEF_PMAX, text, debug_mode)
    )

    # IP rating
    out["ip_rating"] = debug_regex_field("IP (panel)", PAT_IP_PANEL, text, debug_mode)
...
def extract_inversor(text: str, debug_mode: bool) -> Dict[str, Optional[str]]:
    out = {k: None for k in INV_FIELDS}

    # Potencia AC nominal
    raw_nom = debug_regex_field("Potencia AC Nominal", PAT_P_AC_NOM, text, debug_mode)
    pot_nom = to_float_safe(raw_nom)
    out["potencia_AC_nominal_W"] = pot_nom

    # Potencia AC máxima
    raw_max = debug_regex_field("Potencia AC Máxima", PAT_P_AC_MAX, text, debug_mode)
    pot_max = to_float_safe(raw_max)
    out["potencia_AC_max_W"] = pot_max

    # MPPT
    mppt_raw = debug_regex_field("Cantidad MPPT", PAT_MPPT_CNT, text, debug_mode)
    try:
        out["mppt_cantidad"] = int(float(mppt_raw)) if mppt_raw else None
    except ValueError:
        out["mppt_cantidad"] = None

    # Lado DC
    out["Vdc_max"] = to_float_safe(
        debug_regex_field("Vdc Máximo", PAT_VDC_MAX, text, debug_mode)
    )
    out["isc_max_por_mppt_A"] = to_float_safe(
        debug_regex_field("Isc por MPPT", PAT_ISC_PER_MPPT, text, debug_mode)
    )

    # Lado AC
    out["salida_corriente_max_A"] = to_float_safe(
        debug_regex_field("Corriente AC Máxima", PAT_AC_OUT_CURRENT_MAX, text, debug_mode)
    )
    out["tipo_fase"] = guess_phase(text)
    out["eficiencia_max_pct"] = to_float_safe(
        debug_regex_field("Eficiencia Máx", PAT_EFF_MAX, text, debug_mode)
    )
    out["eficiencia_CEC_pct"] = to_float_safe(
        debug_regex_field("Eficiencia CEC/Euro", PAT_EFF_CEC, text, debug_mode)
    )
    ...
    # Frecuencia
    out["frecuencia_Hz"] = to_float_safe(
        debug_regex_field("Frecuencia", PAT_FREQ, text, debug_mode)
    )

    # PF e IP
    out["pf_range"] = debug_regex_field("Rango PF", PAT_PF_RANGE, text, debug_mode)
    out["ip_rating"] = debug_regex_field("IP (inversor)", PAT_IP_INV, text, debug_mode)
...
def main():
    ap = argparse.ArgumentParser(
        description="Extractor de especificaciones solares (paneles/inversores) desde PDFs (V4 HÍBRIDO + DEBUG)"
    )
    ...
    for pdf_path in sorted(in_dir.glob("**/*.pdf")):
        try:
            ...
            if tipo_forzado == "panel":
                ...
                tmp_panel = extract_panel(txt_aug, debug_mode)
                tmp_inv = extract_inversor(txt_aug, debug_mode)

                core_panel = ["potencia_nominal_W", "Voc_V", "Isc_A", "Vmp_V", "Imp_A"]
                core_inv = [
                    "potencia_AC_nominal_W", "Vdc_max",
                ...
                if score_panel == 0 and score_inv == 0:
                    ...
                    panel_score = sum(t.count(k) for k in [
                        "pmax", "pmpp", "wp", "module", "panel", "módulo", "voc",
                        "open circuit voltage", "voltaje de circuito abierto",
                        "isc", "short circuit current", "corriente de corto circuito",
                        "vmp", "voltage at pmax", "maximum power voltage",
                        "imp", "current at pmax", "maximum power current",
                        "dimensiones", "dimensions", " mm ", "×", " x "
                    ])
            ...
            if not is_panel and not is_inversor:
                ...
                tmp_panel2 = extract_panel(txt_aug, debug_mode)
                if any(tmp_panel2.get(k) for k in ["Voc_V", "Isc_A", "Vmp_V", "Imp_A"]):
                    is_panel = True
                    row_panel = tmp_panel2
                else:
                    tmp_inv2 = extract_inversor(txt_aug, debug_mode)
                    if any(tmp_inv2.get(k) for k in [
                        "Vdc_max", "mppt_cantidad",
                        "salida_corriente_max_A", "potencia_AC_nominal_W"
                    ]):
                        is_inversor = True
                        row_inv = tmp_inv2
                    else:
            ...
            if is_panel:
                if row_panel is None:
                    row_panel = extract_panel(txt_aug, debug_mode)
                row = row_panel
                row["fabricante"] = fabricante
                row["modelo"] = modelo
                row["ficha_tecnica_url"] = url

                # Rellenar "no_se_encuentra" y avisar
                row_fixed = mark_missing_and_warn(
                    "PANEL", pdf_path.name, row, PANEL_FIELDS, debug_mode
                )
                panel_rows.append(row_fixed)

                if debug_mode:
                    print(f"[DEBUG][PANEL] Pmax={row_fixed.get('potencia_nominal_W')} Voc={row_fixed.get('Voc_V')} "
                          f"Isc={row_fixed.get('Isc_A')} Vmp={row_fixed.get('Vmp_V')} Imp={row_fixed.get('Imp_A')}")

            elif is_inversor:
                if row_inv is None:
                    row_inv = extract_inversor(txt_aug, debug_mode)
                row = row_inv
                row["fabricante"] = fabricante
                row["modelo"] = modelo
                row["ficha_tecnica_url"] = url
                ...
                row_fixed = mark_missing_and_warn(
                    "INVERSOR", pdf_path.name, row, INV_FIELDS, debug_mode
                )
                inv_rows.append(row_fixed)

                if debug_mode:
                    print(f"[DEBUG][INV] Vdc_max={row_fixed.get('Vdc_max')} "
                          f"MPPT={row_fixed.get('mppt_cantidad')} IoutMax={row_fixed.get('salida_corriente_max_A')}")
